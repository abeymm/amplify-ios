...
class SQLModelValueConverterTests: BaseDataStoreTests {
    ...
    func testInsertAndSelectExampleWithEveryType() async {
        let example = exampleModel
        let saveResult = await Amplify.DataStore.save(example)
        
        
        switch saveResult {
        case .success:
            let queryResult = await Amplify.DataStore.query(ExampleWithEveryType.self, byId: example.id)
